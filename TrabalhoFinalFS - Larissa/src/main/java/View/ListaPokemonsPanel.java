...
public class ListaPokemonsPanel extends JInternalFrame {
    ...
    private void buscarPorTreinador() {
        String nomeBusca = txtBuscarTreinador.getText().trim();
        tableModel.setRowCount(0);

        Pokemon treinador = controller.buscarTreinadorPorNome(nomeBusca);

        if (treinador.getNome().isEmpty() && !nomeBusca.isEmpty()) {
